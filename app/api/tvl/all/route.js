...
export async function GET() {
  try {
    ...
    // Fetch TVL data for each DAO
    const results = await Promise.all(
      DAO_GOVERNANCE_IDS.map(async (daoGovernanceProgramId) => {
        const response = await fetch(`${baseUrl}/api/tvl/${daoGovernanceProgramId}`);
        if (!response.ok) {
          throw new Error(`Failed to fetch TVL for ${daoGovernanceProgramId}`);
        }
    ...
    const totalValue = results.reduce((sum, { totalValue }) => sum + parseFloat(totalValue), 0).toFixed(2);

    // Add the total sum as the first object in the array
    const tvlData = [
      {
